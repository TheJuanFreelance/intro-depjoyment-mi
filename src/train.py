...
grid_search = GridSearchCV(
    model, param_grid=param_tunning, scoring='r2', cv=10, refit=True, n_jobs=2)
...
final_result = cross_validate(
    grid_search.best_estimator_, X_train, y_train, cv=10, return_train_score=True, n_jobs=2)

train_score = np.mean(final_result['train_score'])
validation_score = np.mean(final_result['test_score'])  # validate
